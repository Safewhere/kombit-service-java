...
public class SOAPException extends IOException {
     ...
     *
     * @param name
     * @return Can return <code>null</code> if the response did not contain a
     * fault element.
     */
    public Fault getFault(QName name) {
        if (fault != null)
            return fault;
        ...
        List<XMLObject> faults = envelope.getBody().getUnknownXMLObjects(name);
        if (faults.isEmpty()) {
            return null;
        }
    ...
    /**
     * Get the SOAP Fault object by default name
     * 
     * @return 
     */
    public Fault getFault(){
        return getFault(Fault.DEFAULT_ELEMENT_NAME);
    }
    
    /**
     * Set a specific SOAP fault object to SOAPException
     * 
     * @param aFault 
     */
    public void setFault(Fault aFault){
        this.fault = aFault;
    }
}
