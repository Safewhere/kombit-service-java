...
public class OIOAssertion extends OIOSamlObject {
    ...
    /**
     * Check whether an assertion contains an assertionConsumerURL within a
     * subjectConfirmationData having the
     * subjectConfirmationMethod=urn:oasis:names:tc:SAML:2.0:cm:bearer
     *
     * @return <code>true</code>, if the assertion contains the
     * assertionConsumerURL. <code>false</code> otherwise.
     */
    public boolean checkRecipient(String assertionConsumerURL) {
        ...
        for (SubjectConfirmation subjectConfirmation : assertion.getSubject().getSubjectConfirmations()) {
            if (!OIOSAMLConstants.METHOD_BEARER.equals(subjectConfirmation.getMethod())) {
                continue;
            }
     ...
     *
     * @return <code>true</code>, if the assertion has expired.
     ...
     *
     * @param validator The validator to use forassertion validation. Can be
     * <code>null</code>.
     * @param spEntityID The entityID of the service provider
     * @param spAssertionConsumerURL The assertion consumer URL of the service
     * provider
     */
