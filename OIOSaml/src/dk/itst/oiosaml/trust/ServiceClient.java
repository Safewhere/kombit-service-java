...
/**
 * Client interface for invoking identity based webservices according to the
 * Liberty Basic SOAP Profile.
 *
 * @author Joakim Recht
 *
 */
public class ServiceClient extends ClientBase {
     ...
     *
     * This requires OIOSAML.java to be configured, and will use the credentials
     * specified in oiosaml-sp.properties.
     *
     * @param credential
     * @param logFilePath
     */
    public ServiceClient(X509Credential credential, String logFilePath) {
        super(credential, logFilePath);
    }
     ...
     *
     * <p>
     * A SOAP header is added automatically to the request containing the
     * client's security token.</p>
     * <p>
     * If {@link #getToken()} has been called, the retrieved token is added to
     * the request automatically.</p>
     *
     * <p>
     * SOAP Faults can be handled by adding {@link FaultHandler}s to the
     * client.</p>
     *
     ...
     * @param verificationKey Key to use for signature verification on the
     * response. If <code>null</code>, the signature is not checked. If the
     * signature is not valid, a {@link TrustException} is thrown.
     * @param resultHandler When the request has been completed, the response
     * will be sent to this callback. The handler can be <code>null</code>, in
     * which case the response is ignored.
     * @throws InvocationTargetException Thrown when an exception is thrown in a
     * handler.
     * @throws TrustException If an unhandled SOAP Fault occurs, or if a
     * transport error occurs.
     */
    public void sendRequest(XMLObject body, String location, String action, PublicKey verificationKey, ResultHandler<XMLObject> resultHandler) throws InvocationTargetException, ClassNotFoundException, InvalidTransformException {
        ...
        try {
            ...
            MessageLogger.LogToFile(getLogFilePath(), "Java.Consumer.SentRequestToService.xml", getLastRequestXML());
            OIOSoapEnvelope res = new OIOSoapEnvelope(soapClient.wsCall(location, true, getLastRequestXML(), action));
            setLastResponse(res);
            ...
            MessageLogger.LogToFile(getLogFilePath(), "Java.Consumer.ReceivedResponseFromService.xml", res.toXML());
            ...
            if (fault != null && fault.getDetail() != null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("No handler for fault " + e);
                }
                throw new TrustException(e);
            }
        } catch (NoSuchAlgorithmException | InvalidAlgorithmParameterException | MarshalException | XMLSignatureException | IOException e) {
            throw new TrustException(e);
        }
    }

    public void sendRequest(Element body, String location, String action, PublicKey verificationKey, final ResultHandler<Element> resultHandler) throws InvocationTargetException, ClassNotFoundException, InvalidTransformException {
        try {
     ...
     *
     * @param <T> The response type. No type checking is performed, so if this
     * type is not correct, a {@link ClassCastException} will occur.
    ...
    public <T> void sendRequest(Object body, final JAXBContext context, String location, String action, PublicKey verificationKey, final ResultHandler<T> resultHandler) {
        ...
        } catch (JAXBException | ParserConfigurationException | ClassNotFoundException | InstantiationException | IllegalAccessException | NoSuchMethodException | SecurityException | IllegalArgumentException | InvocationTargetException | UnmarshallingException | InvalidTransformException e) {
            throw new RuntimeException(e);
        }
    ...
    /**
     * Configure whether ws requests should be signed or not. Token requests are
     * always signed.
     *
