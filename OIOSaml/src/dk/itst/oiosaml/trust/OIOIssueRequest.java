...
import dk.itst.oiosaml.liberty.ClaimValue;
...
public class OIOIssueRequest {
    ...
    /**
     * Create a new request with empty request.
     *
     * This builds a new request and just contain empty RequestSecurityToken .
     *
     * @return
     */
    public static OIOIssueRequest buildEmptyRequest() {
        RequestSecurityToken req = SAMLUtil.buildXMLObject(RequestSecurityToken.class);
        return new OIOIssueRequest(req);
    }
    ...
    public void setIssuer(String issuer) {
        ...
        ref.setAddress(issuerAddress);
        i.getUnknownXMLObjects().add(ref);
    ...
    /**
     * Set the assertion id of the attached assertion. The assertion must be
     * placed in the Security header as a SecurityTokenReference. The assertion
     * is included in a wst:OnBehalfOf element.
     * @param assertionId
     */
    ...
    /**
     * Include the assertion in a wst:ActAs element. The assertion itself must
     * be placed in the Security header.
     * @param assertionId
     */
    ...
    /**
     * Set the onbehalfof value. This adds the input structure directly to
     * OnBehalfOf.
     * @param obj
     */
     ...
     * @param claimObjects The contents of the Claims element. Any objects will
     * do.
     */
     ...
     * @param claims The list of claims to include, it's a pair of claim(uri, value). These are included as
     * ic:ClaimType elements with an Uri attribute.
     */
    public void setClaims(String dialect, Map<String,String> claims) {
        ...
        for (String key: claims.keySet()) {
            ...
            ct.setUri(key); 
            ClaimValue cv = SAMLUtil.buildXMLObject(ClaimValue.class);
            cv.setValue(claims.get(key));
            ct.getUnknownXMLObjects().add(cv);
     ...
     * @param dialect The dialect to use. Can be <code>null</code>.
     * @param claim
     * @param claims The list of claims to include. These are included as
     * ic:ClaimType elements with an Uri attribute.
     */
    public void setClaimWithValue(String dialect, String claim, String claimValue) {
        Claims c = SAMLUtil.buildXMLObject(Claims.class);
        c.setDialect(dialect);        
        ClaimType ct = SAMLUtil.buildXMLObject(ClaimType.class);
        ct.setUri(claim); 
